...
// node_modules/.pnpm/tsup@5.12.5_typescript@4.5.4/node_modules/tsup/assets/cjs_shims.js
...
// node_modules/.pnpm/defu@6.0.0/node_modules/defu/dist/defu.mjs
init_cjs_shims();
...
// node_modules/.pnpm/load-tsconfig@0.2.3/node_modules/load-tsconfig/dist/index.js
init_cjs_shims();
...
var loadTsConfigInternal = (dir = process.cwd(), name = "tsconfig.json", files = [], isExtends = false) => {
  ...
  if ((_a = data.compilerOptions) == null ? void 0 : _a.baseUrl) {
    data.compilerOptions.baseUrl = import_path2.default.join(configDir, data.compilerOptions.baseUrl);
  }
  if (data.extends) {
...
async function pathExists(path4) {
  try {
    await import_node_fs.promises.access(path4);
    return true;
  } catch {
    return false;
  }
...
var resolveFile = async (resolved, index = false) => {
  for (const ext of RESOLVE_EXTENSIONS) {
    const file = index ? import_path3.default.join(resolved, `index${ext}`) : `${resolved}${ext}`;
...
var resolveId = async (importee, importer) => {
  if (importer && importee[0] === ".") {
    const absolutePath = import_path3.default.resolve(importer ? import_path3.default.dirname(importer) : process.cwd(), importee);
    let resolved = await resolveFile(absolutePath);
    if (!resolved && await pathExists(absolutePath) && await import_fs2.default.promises.stat(absolutePath).then((stat) => stat.isDirectory())) {
...
// src/index.ts
var src_default = (0, import_unplugin.createUnplugin)((_a = {}) => {
  var _b = _a, { tsconfigFile, minify, include, exclude } = _b, options = __objRest(_b, ["tsconfigFile", "minify", "include", "exclude"]);
  ...
  return {
    ...
    async transform(code, id) {
      ...
      if (!filter(id))
        return null;
      const compilerOptions = tsconfigFile === false ? {} : ((_b2 = (_a2 = loadTsConfig(import_path4.default.dirname(id), tsconfigFile === true ? void 0 : tsconfigFile)) == null ? void 0 : _a2.data) == null ? void 0 : _b2.compilerOptions) || {};
      ...
      const result = await (0, import_core.transform)(code, __spreadProps(__spreadValues({
        filename: id,
        sourceMaps: true
      }, options), {
        jsc
