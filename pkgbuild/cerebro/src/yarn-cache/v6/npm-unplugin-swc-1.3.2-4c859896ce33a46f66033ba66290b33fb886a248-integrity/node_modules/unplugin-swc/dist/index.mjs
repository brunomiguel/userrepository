...
// node_modules/.pnpm/tsup@5.12.5_typescript@4.5.4/node_modules/tsup/assets/esm_shims.js
...
var resolveFile = async (resolved, index = false) => {
  for (const ext of RESOLVE_EXTENSIONS) {
    const file = index ? path2.join(resolved, `index${ext}`) : `${resolved}${ext}`;
...
var resolveId = async (importee, importer) => {
  if (importer && importee[0] === ".") {
    const absolutePath = path2.resolve(importer ? path2.dirname(importer) : process.cwd(), importee);
    let resolved = await resolveFile(absolutePath);
    if (!resolved && await pathExists(absolutePath) && await fs3.promises.stat(absolutePath).then((stat) => stat.isDirectory())) {
...
// src/index.ts
var src_default = createUnplugin((_a = {}) => {
  ...
  return {
    ...
    async transform(code, id) {
      ...
      if (!filter(id))
        return null;
      const compilerOptions = tsconfigFile === false ? {} : ((_b2 = (_a2 = loadTsConfig(path3.dirname(id), tsconfigFile === true ? void 0 : tsconfigFile)) == null ? void 0 : _a2.data) == null ? void 0 : _b2.compilerOptions) || {};
      ...
      const result = await transform(code, __spreadProps(__spreadValues({
        filename: id,
        sourceMaps: true
      }, options), {
        jsc
