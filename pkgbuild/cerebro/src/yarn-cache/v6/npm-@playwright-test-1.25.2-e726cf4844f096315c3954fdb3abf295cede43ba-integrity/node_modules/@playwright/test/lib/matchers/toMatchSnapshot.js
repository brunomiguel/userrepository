...
async function toHaveScreenshot(pageOrLocator, nameOrOptions = {}, optOptions = {}) {
  ...
  const snapshotPathResolver = process.env.PWTEST_USE_SCREENSHOTS_DIR_FOR_TEST ? testInfo._screenshotPath.bind(testInfo) : testInfo.snapshotPath.bind(testInfo);
  ...
  if (this.isNot) {
    if (!hasSnapshot) return helper.handleMissingNegated(); // Having `errorMessage` means we timed out while waiting
  ...
  if (!hasSnapshot) {
    ...
    if (errorMessage) return helper.handleDifferent(actual, undefined, previous, diff, undefined, log, errorMessage); // We successfully generated new screenshot.
