...
const { out: flagOut, project = 'tsconfig.json', src: flagSrc, verbose = false, } = program;
...
const outFileToSrcFile = (x) => (0, path_1.resolve)(usingSrcDir, (0, path_1.relative)(outPath, x));
const aliases = Object.keys(paths)
    .map(alias => ({
    prefix: alias.replace(/\*$/, ''),
    aliasPaths: paths[alias].map(p => (0, path_1.resolve)(basePath, p.replace(/\*$/, ''))),
}))
...
const absToRel = (modulePath, outFile) => {
    ...
    for (let j = 0; j < alen; j += 1) {
        ...
        if (modulePath.startsWith(prefix)) {
            ...
            verboseLog(`${outRel} (source: ${(0, path_1.relative)(basePath, srcFile)}):`);
            ...
            for (let i = 0; i < len; i += 1) {
                ...
                if ((0, fs_1.existsSync)(moduleSrc) || exts.some(ext => (0, fs_1.existsSync)(moduleSrc + ext))) {
                    const rel = toRelative((0, path_1.dirname)(srcFile), moduleSrc);
                    replaceCount += 1;
                    verboseLog(`\treplacing '${modulePath}' -> '${rel}' referencing ${(0, path_1.relative)(basePath, moduleSrc)}`);
                    return rel;
                }
            }
